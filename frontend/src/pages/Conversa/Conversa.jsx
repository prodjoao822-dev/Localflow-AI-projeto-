 ...
 * Estrutura unificada: este componente NÃO é mais uma página com rota própria.
 * Ele renderiza as colunas CENTRAL (thread) e DIREITA (contexto) da tela de
 * Atendimento, dentro da mesma página que a fila (PainelAtendente).
 *
 * - Recebe `conversaId` via props (seleção de estado, sem useParams/Link).
 * - Recebe `aoAtualizar` para refletir mutações (assumir/enviar/status) na fila.
 ...
 */
export default function Conversa({ conversaId, aoAtualizar }) {
  ...
  useEffect(() => {
    ...
    setCarregando(true);
    setConversa(null);
    conversaService.buscarConversaPorId(conversaId).then((dados) => {
      if (!ativo) return;
      if (!dados) {
        setCarregando(false);
        setConversa(null);
  ...
  return (
    <div className="conversa">
      {/* Topo da conversa (sem "voltar para a fila" — a fila está sempre ao lado) */}
      ...
      {/* Corpo: mensagens + interações da IA (thread) + contexto (lateral) */}
